...
Ingest and fuse at least three signal sources, such as social posts, weather, traffic, emergency calls, mock sensors, field reports, and historical data.
Detect and classify crisis type, location, severity, confidence, affected population, expected duration, and likely evolution.
Prioritize and allocate constrained response resources across one or more simultaneous crises.
Simulate coordinated actions such as traffic rerouting, emergency dispatch, hospital preparation, utility escalation, and public alerts.
Predict outcomes, side effects, and unintended consequences for each response action. Handle false positives, false negatives, and conflicting signals through verification and escalation logic.
...
Working prototype with a mobile app as mandatory and web app/dashboard as optional.
Demo video of 3-5 minutes showing multi-source input -> crisis detection -> severity prediction -> resource allocation -> simulated response -> impact visualization -> recovery scenario.
Antigravity agent trace/logs showing signal fusion, confidence scoring, crisis classification, allocation trade-offs, stakeholder messages, action execution, and fallback behavior.
README with architecture, data stream schemas, Antigravity usage, APIs/tools, assumptions, privacy/safety note, cost/latency analysis, baseline comparison, scalability discussion, and limitations.
EVALUATION CRITERIA
